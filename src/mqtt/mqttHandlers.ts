...
export function handleGlobMessage(
  ...
  for (const s of msg.t) {
    ...
    const globState: DoorState = s.val === 1 ? "open" : "closed";
...
export function handleTofEventMessage(
  ...
  console.log(
    `[HYSTERESIS] ${sensorId}: ${runtime.previous_state} → ${runtime.current_state}`
  );
...
function finalizeHysteresis(store: SensorStore, sensorId: number, eventEngine: EventEngine) {
  const runtime = store.getRuntimeState(sensorId);
  if (!runtime) return;
  if (!runtime.hysteresis_target_state) return;
